(function() {
  ...
  Lexer = (function() {
    var BETWEEN, BLOCKCOMMENT, BOOLEAN, COMMA, DBLSTRING, EOLCOMMENT, LITERAL, MATH, MATH_MULTI, NUMBER, PARAMETER, SEMICOLON, SQL_CONDITIONALS, SQL_OPERATORS, SQL_SORT_ORDERS, STAR, STRING, SUB_SELECT_OP, SUB_SELECT_UNARY_OP, UNKNOWN, WHITESPACE;
    ...
    class Lexer {
      ...
      tokenizeFromStringRegex(name, regex, part = 0, lengthPart = part, output = true) {
        var match, partMatch;
        if ((match = regex.exec(this.chunk)) == null) {
          return 0;
        }
        partMatch = match[part].replace(/''/gu, "'");
        if (output) {
          this.push_token(name, partMatch);
        }
      ...
      eolcommentToken() {
        return this.tokenizeFromRegex('EOLCOMMENT', EOLCOMMENT);
      }

      blockcommentToken() {
        return this.tokenizeFromRegex('BLOCKCOMMENT', BLOCKCOMMENT);
      }
      ...
      whitespaceToken() {
        ...
        if (this.keep_whitespace) {
          this.push_token('WHITESPACE', partMatch);
        }
